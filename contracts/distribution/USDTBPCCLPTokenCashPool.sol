...
// USDTBPCCLPTokenSharePool is the master of Sushi. He can make Sushi and he is a fair guy.
//
...
contract USDTBPCCLPTokenCashPool is Ownable {
    ...
    modifier checkStart() {
        require(
            ...
            'USDTBPCCLPTokenSharePool: not start'
        );
        _;
    }
    ...
    // Deposit LP tokens to USDTBPCCLPTokenSharePool for CASH allocation.
    function stake(uint256 _amount) public checkStart {
        require(_amount > 0, 'USDTBPCCLPTokenSharePool: Cannot stake 0');
    ...
    // Withdraw LP tokens from USDTBPCCLPTokenSharePool.
    function withdraw(uint256 _amount) public checkStart {
        require(_amount > 0, 'USDTBPCCLPTokenSharePool: Cannot withdraw 0');
        UserInfo storage user = userInfo[msg.sender];
    ...
    function exit() external {
        UserInfo storage user = userInfo[msg.sender];
        require(user.amount > 0, 'USDTBPCCLPTokenSharePool: No stake');
