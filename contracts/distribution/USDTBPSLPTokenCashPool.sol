...
// USDTBPSLPTokenCashPool is the master of Sushi. He can make Sushi and he is a fair guy.
//
...
contract USDTBPSLPTokenCashPool is Ownable {
    ...
    modifier checkStart() {
        require(
            ...
            'USDTBPSLPTokenCashPool: not start'
        );
        _;
    }
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (lastTimeRewardApplicable() > poolInfo.lastRewardTime && lpSupply != 0) {
            ...
            uint256 totalReward = multiplier.mul(rewardSupply).div(remainTime);
            uint256 devReward = totalReward.mul(5).div(100);
            uint256 reward = totalReward.sub(devReward);
    ...
    function updatePool() public {
        ...
        uint256 totalReward = multiplier.mul(rewardSupply).div(remainTime);
        uint256 devReward = totalReward.mul(5).div(100);
        cash.safeTransferFrom(supplier, devaddr, devReward);
        uint256 reward = totalReward.sub(devReward);
    ...
    // Deposit LP tokens to USDTBPSLPTokenCashPool for CASH allocation.
    function stake(uint256 _amount) public checkStart {
        require(_amount > 0, 'USDTBPSLPTokenCashPool: Cannot stake 0');
    ...
    // Withdraw LP tokens from USDTBPSLPTokenCashPool.
    function withdraw(uint256 _amount) public checkStart {
        require(_amount > 0, 'USDTBPSLPTokenCashPool: Cannot withdraw 0');
        UserInfo storage user = userInfo[msg.sender];
    ...
    function exit() external {
        UserInfo storage user = userInfo[msg.sender];
        require(user.amount > 0, 'USDTBPSLPTokenCashPool: No stake');
